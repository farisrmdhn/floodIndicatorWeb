...
<?php if(!$notifications): ?>
	<div class="row">
		<div class="col-md-12">	
			<h3 style="margin-top: 30px; color: grey;">No Notification</h3>
		</div>
	</div>
<?php else:?>
<div class="row">
			...
			<tbody>
				<?php foreach($notifications as $notification):?>
					<tr>
						...
						<td>
							<?php if($notification['is_new'] == 'true'):?>
							<form method="post" action="<?php echo base_url();?>notifications/update_notification">
								<input type="hidden" name="id" value="<?php echo $notification['id']?>">
								<input type="submit" name="submit" value="OK" class="btn btn-success">
							</form>
							<?php else:?>
							<form method="post" action="<?php echo base_url();?>notifications/delete_notification">
								<input type="hidden" name="id" value="<?php echo $notification['id']?>">
